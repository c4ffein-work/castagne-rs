...
mod tests {
    ...
    fn validate_golden_master_structure(golden: &Value) {
        assert!(
            ...
            "Golden master should have metadata"
        );
        assert!(
            golden["variables"].is_object(),
            "Golden master should have variables"
        );
        assert!(
            golden["states"].is_object(),
            "Golden master should have states"
        );
        assert!(
            golden["subentities"].is_object(),
            "Golden master should have subentities"
        );
        assert!(
            golden["transformed_data"].is_object(),
            "Golden master should have transformed_data"
        );
    }
    ...
    fn test_golden_master_structure_test_character_complete() {
        ...
        assert_eq!(
            metadata["author"].as_str().unwrap(),
            "Parser Development Team"
        );
        assert_eq!(
        ...
        assert_eq!(
            metadata["filepath"].as_str().unwrap(),
    ...
    fn test_golden_master_structure_baston_model() {
        ...
        assert_eq!(
            metadata["editorname"].as_str().unwrap(),
            "Baston Labatte (Custom Character)"
        );
    ...
    fn test_golden_master_structure_baston_2d() {
        ...
        assert_eq!(
            metadata["editorname"].as_str().unwrap(),
            "Baston 2D (Example Character)"
        );
        assert_eq!(
            metadata["skeleton"].as_str().unwrap(),
    ...
    fn example_full_parser_comparison() {
        ...
        */
        ...
        println!(
            ...
            golden["variables"].as_object().unwrap().len()
        );
        println!(
            "  Expected states count: {}",
            golden["states"].as_object().unwrap().len()
        );
    }
        ...
        match (rust_value, golden_value) {
            (Value::Object(rust_obj), Value::Object(golden_obj)) => {
                ...
                for key in golden_obj.keys() {
                    if !rust_obj.contains_key(key) {
                        differences
                            .push(format!("{}.{}: missing in Rust parser output", path, key));
                    }
                }
                ...
                for key in rust_obj.keys() {
                    if !golden_obj.contains_key(key) {
                        differences.push(format!(
                            "{}.{}: unexpected key in Rust parser output",
                            path, key
                        ));
                    }
                }
            ...
            (Value::Array(rust_arr), Value::Array(golden_arr)) => {
                if rust_arr.len() != golden_arr.len() {
                    differences.push(format!(
                        "{}: array length mismatch (Rust: {}, Golden: {})",
                        path,
                        rust_arr.len(),
                        golden_arr.len()
                    ));
                    ...
                    for (i, (rust_item, golden_item)) in
                        rust_arr.iter().zip(golden_arr.iter()).enumerate()
                    {
    ...
    fn compare_metadata(rust_json: &Value, golden_json: &Value) -> Vec<String> {
        ...
        for field in fields {
            ...
            if rust_val != golden_val {
                differences.push(format!(
                    "metadata.{}: Rust={:?}, Golden={:?}",
                    field, rust_val, golden_val
                ));
            }
        }
    ...
    fn compare_structure_sizes(rust_json: &Value, golden_json: &Value) -> Vec<String> {
        ...
        for section in sections {
            ...
            let golden_count = golden_json[section]
                .as_object()
                .map(|o| o.len())
                .unwrap_or(0);
    ...
    fn validate_transformed_data_module(module_data: &Value, module_name: &str) -> Vec<String> {
        ...
        if !module_data["Defines"].is_object() {
            issues.push(format!(
                "{}: missing or invalid 'Defines' section",
                module_name
            ));
        }
    ...
    fn test_comparison_helpers() {
        ...
        assert_eq!(
            diffs.len(),
            0,
            "Identical values should have no differences"
        );
    ...
    fn test_metadata_comparison_helper() {
        ...
        assert_eq!(
            diffs.len(),
            0,
            "Comparing golden master with itself should show no differences"
        );
    ...
    fn test_structure_size_comparison() {
        ...
        assert_eq!(
            diffs.len(),
            0,
            "Structure sizes should match when comparing with itself"
        );
    ...
    fn test_edge_case_empty_states() {
        ...
        println!(
            "✓ Found {} states with empty phases (valid edge case)",
            empty_phase_count
        );
    ...
    fn test_edge_case_state_name_lengths() {
        ...
        println!(
            "  Longest state name: '{}' ({} chars)",
            longest_name, max_name_len
        );
        ...
        for (module_name, module_data) in transformed_data.iter() {
            ...
            if module_data.is_object() {
                ...
                if defines.is_object() {
                    ...
                    if define_count > 50 {
                        println!(
                            "  Module '{}' has {} defines (deep structure)",
                            module_name, define_count
                        );
                    }
                }
            }
    ...
    fn test_cross_validation_golden_masters() {
        ...
        for file in &golden_files {
            ...
            println!(
                "  {}: metadata={}, vars={}, states={}, subent={}, trans={}",
                file, structure.0, structure.1, structure.2, structure.3, structure.4
            );
        }

        // All should have the same structure
        assert!(
            structures.iter().all(|s| *s == structures[0]),
